...
use super::gcp_signature::{GCPCredentials, GCPSignature, GCPSignatureRequest};
use super::gcs;
...
pub async fn proxy(
    ...
    let client_authorization_header = client_request_headers
        ...
        .to_str()?;
    let aws_v4_auth_header =
        AWSV4AuthHeader::from_header(client_authorization_header)?;
    if aws_v4_auth_header.credential.hmac_access_key
        != state.config.hmac_access_key
    {
    ...
    let proxy_request_query =
        client_request.uri().query().unwrap_or("").to_string();
    ...
    let gcp_signature_request = GCPSignatureRequest {
        ...
        csek_encryption_key_sha256: state
            .config
    ...
    let mut proxy_request_headers = client_request_headers
        ...
        .filter_map(|(k, v)| {
            k.and_then(|key| {
                (!key.as_str().starts_with(gcs::AMZ_HEADER_KEY_PREFIX))
                    .then_some((key, v))
            })
        })
        .collect::<HeaderMap>();
    ...
    proxy_request_headers
        .insert(http::header::HOST, HeaderValue::from_static(gcs::HOST));
    proxy_request_headers.insert(
        http::header::AUTHORIZATION,
        HeaderValue::from_str(&auth_header)?,
    );
    proxy_request_headers
        .insert(gcs::DATE_HEADER_KEY, HeaderValue::from_str(&goog_date)?);
    proxy_request_headers.insert(
    ...
    proxy_request_headers.insert(
        ...
        HeaderValue::from_str(&state.config.csek_encryption_key_sha256)?,
    );
    ...
    if proxy_request_body_bytes_length > 0 {
        let validation = validate_response(
            ...
            &state.config.csek_encryption_key_sha256,
        );
    ...
    let proxy_response_body_stream =
        axum::body::Body::from_stream(server_response_body_stream);
    ...
    match response_headers.get(gcs::CSEK_ENCRYPTION_ALGORITHM_HEADER_KEY) {
        Some(header) => {
            if header.to_str()? != gcs::CSEK_ENCRYPTION_ALGORITHM {
                anyhow::bail!(
                    "Unexpected encryption algorithm in server response."
                );
            } else {
                header.to_str()?
            }
        ...
        None => anyhow::bail!(
            "Couldn't find encryption algorithm header in server response"
        ),
    };
    ...
    match response_headers.get(gcs::CSEK_ENCRYPTION_KEY_SHA256_HEADER_KEY) {
        Some(header) => {
            if header.to_str()? != csek_base64_of_sha256_of_encryption_key {
                anyhow::bail!(
                    "Unexpected encryption key hash in server response."
                );
            } else {
                header.to_str()?
            }
        ...
        None => anyhow::bail!(
            "Couldn't find encryption key SHA256 header in server response."
        ),
    };
    Ok(())
}
...
fn validate_multipart_upload_finish_headers(
    response_headers: &HeaderMap,
) -> bool {
